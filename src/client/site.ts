(function () {
  ...
  const headerSection = document.querySelector<HTMLElement>('.running-header__section');
  if (headerSection) {
    const sections = document.querySelectorAll<HTMLElement>('article section[data-title], article h2[id]');
    if (sections.length) {
      const observer = new IntersectionObserver(entries => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            const element = entry.target as HTMLElement;
            headerSection.textContent = element.dataset.title || element.textContent?.trim().slice(0, 60) || '';
          }
        });
  ...
  function onScroll() {
    if (!ticking) {
      requestAnimationFrame(() => {
        ...
        document.documentElement.style.setProperty('--wght', String(weight));
        ticking = false;
      });
      ticking = true;
    }
  ...
  document.addEventListener('click', event => {
    ...
    if (hasWideMargin.matches) {
      let note = document.querySelector<HTMLElement>(`.margin-note[data-for="${targetId.replace(/^#/, '')}"]`);
      if (!note) {
    ...
    let inline = document.getElementById(`inline-${targetId.replace(/^#/, '')}`);
    if (!inline) {
